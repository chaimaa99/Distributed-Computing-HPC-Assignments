...
int64_t bind_c_solve_1d_diff_pyccel(int64_t n0_u, double *u, int64_t n0_un, double *un, int64_t nt, int64_t nx, double dt, double dx, int64_t nu);
...
PyObject *solve_1d_diff_pyccel_wrapper(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (PyArray_IsScalar(nu_tmp, Int64))
    {
        PyArray_ScalarAsCtype(nu_tmp, &nu);
    }
    else if (PyLong_Check(nu_tmp))
    {
        nu = PyLong_AsLong(nu_tmp);
    }
    else
    {
        PyErr_SetString(PyExc_TypeError, "\"nu must be 64 bit int\"");
        return NULL;
    }
    u_dim = PyArray_DIM(u_tmp, 0);
    un_dim = PyArray_DIM(un_tmp, 0);
    Out_0001 = bind_c_solve_1d_diff_pyccel(u_dim, u, un_dim, un, nt, nx, dt, dx, nu);
    result = Py_BuildValue("l", Out_0001);
    return result;
}
/*........................................*/

static PyMethodDef mod_awliwdv5_methods[] = {
    {
...
static struct PyModuleDef mod_awliwdv5_module = {
    ...
    -1,
    mod_awliwdv5_methods
};

/*........................................*/

PyMODINIT_FUNC PyInit_mod_awliwdv5(void)
{
    PyObject *m;

    import_array();

    m = PyModule_Create(&mod_awliwdv5_module);
